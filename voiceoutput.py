...
class VoiceOutput:
    def __init__(
        ...
        speech_style: str = SpeechStyle.EMPATHETIC,
        rate: float = 0.95,
        pitch: float = 0.0
    ):
    ...
    def _create_ssml(self, text: str, style: Optional[str] = None) -> str:
        emotion = style or self.speech_style
        return f"""
    ...
    async def speak(self, text: str, emotion: Optional[str] = None) -> bool:
        try:
            valid_emotions = [
                "empathetic", "friendly", "gentle",
                "cheerful", "supportive", "hopeful", "sorry"
            ]
